...
back = (ImageView)findViewById(R.id.go_back);
		forward = (ImageView) findViewById(R.id.go_forward);
        ...
        for(int i = 0;i< getChildCount(); i++){
        	if(getChildAt(i) instanceof TextView){
        		((TextView)getChildAt(i)).setTypeface(android.graphics.Typeface.createFromAsset(getContext().getAssets(), "fonts/product.ttf"));
...
public IVideoPlayer getPlayer(){
	...
	public void setSubtitle(String s){
		try{
		((TourVideoPlayer)mVideoPlayer).getMediaPlayer().addTimedTextSource(getContext(), android.net.Uri.parse(s), android.media.MediaPlayer.MEDIA_MIMETYPE_TEXT_SUBRIP);
			((TourVideoPlayer)mVideoPlayer).getMediaPlayer().setOnTimedTextListener(new android.media.MediaPlayer.OnTimedTextListener(){
					@Override
					public void onTimedText(android.media.MediaPlayer mp, android.media.TimedText text) {
						((TextView)findViewById(R.id.sub)).setText(text.getText());
    ...
    public void onClick(View v) {
    	if(v == back){
			try{
				mVideoPlayer.seekTo(mVideoPlayer.getCurrentPosition() - 10000);
			}catch(Exception e){
	...
	try{
        ...
        if (v == mCenterStart) {
            ...
            if (mVideoPlayer.isPlaying()
                ...
                back.setVisibility(topBottomVisible ? VISIBLE : GONE);
                forward.setVisibility(topBottomVisible ? VISIBLE : GONE);
