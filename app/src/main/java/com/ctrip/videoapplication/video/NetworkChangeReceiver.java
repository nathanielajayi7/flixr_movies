...
import app.flix.free.SketchApplication;
...
public class NetworkChangeReceiver extends BroadcastReceiver {
    ...
    public void registerNetworkChangeBroadcast() {
        ...
        try {
            ...
            SketchApplication.getInstance().registerReceiver(this, filter);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void unRegisterNetworkChangeBroadcast() {
        ...
        SketchApplication.getInstance().unregisterReceiver(this);
        mAtomicBoolean.set(false);
    }

    public static boolean isWifi() {
        return getNetworkStatus(SketchApplication.getInstance()) == TourVideoPlayer.NetworkStatus.CONNECTED_WIFI;
    }

    public static boolean is4G() {
        return getNetworkStatus(SketchApplication.getInstance()) == TourVideoPlayer.NetworkStatus.CONNECTED_4G;
    }

    public static boolean isDisconnect() {
        return getNetworkStatus(SketchApplication.getInstance()) == TourVideoPlayer.NetworkStatus.NOT_CONNECTED;
    }
